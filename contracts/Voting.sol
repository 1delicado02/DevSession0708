...
contract Voting is IForwarder, AragonApp {
    ...
    struct Vote {
        ...
        uint64 supportRequiredPct; // This variable represents the minimum percentage of support required for a proposal to be considered valid or executed
        uint64 minAcceptQuorumPct; // This variable represents the minimum percentage of voting power that needs to be cast in the vote for it to be considered valid.
        uint256 yea;
        uint256 nay;
        uint256 votingPower; // The total voting power (measured as a uint256) of all the eligible voters.
        bytes executionScript; // A script or bytecode that specifies the actions to be executed if the vote is approved.
        mapping (address => VoterState) voters; //  A mapping that keeps track of the voting state of each individual voter.
    }
